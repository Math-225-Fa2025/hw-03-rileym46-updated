...
1.  How many observations (rows) does the dataset have? Instead of hard coding the number in your answer, use inline code. `768 observations.`
...
1.  Run `View(accidents)` in your Console to view the data in the data viewer. What does each row in the dataset represent? `Each row represents an accident.`
...
```{r accidents-daily, eval = TRUE, echo = FALSE, fig.fullwidth = TRUE}
accidents %>%
  mutate(day_of_week_type = if_else(day_of_week %in% c("Saturday", "Sunday"), "Weekend", "Weekday")) %>%
  ggplot(aes(x = time, fill = severity)) +
  geom_density(alpha = 0.5) +
...
```
`The mutate function sets two sections: weekend and weekday. These are the days that the accidents take place. the if_else code has the format (condition, true, false), so in this case, if the accident happened on Saturday or Sunday, it's listed as weekend, and if it was on another day, weekday.` 
`The ggplot sets x = the time of day that an accident happened. The fill color is the severity of the accident, so fatal accidents are purple, serious blue, and slight yellow. This helps us see the distribution of damage. There are more fatal accidents in the early hours of the morning or around noon on weekdays, and very few on weekends. We see more slight accidents around 5 on weekdays and 4 on weekends.`
...
```{r graph, eval = TRUE, echo = FALSE, fig.fullwidth = TRUE}
accidents %>%
  ggplot(aes(x = speed_limit, y = vehicles, color = road_surface)) +
  geom_point()+
  labs(
    Title = "Speed limit and number of vehicles in accidents",
    Subtitle = "Taking into account the road surface",
    x = "Speed Limit",
...
```
`This graph shows the correlation between speed limit and number of vehicles involved in the accident, as well as the road surface. We can see that a majority of higher-speed crashes involve wet or damp ground, but a big amount still include dry. Not many include snow, frost, or ice, suggesting that people drive more cautiously in that weather or just stay home.`
